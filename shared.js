...
function readStr(u8, o, len = -1) {
  let end = o;
  const maxEnd = len === -1 ? u8.length : Math.min(u8.length, o + len);
  while (end < maxEnd && u8[end] !== 0) {
    end++;
  }
  const slice = u8.subarray(o, end);
...
class MemFS {
  constructor(options) {
    ...
    const stdinVal = options.stdinStr || "";
    this.stdinStr = typeof stdinVal === 'string' ? new TextEncoder().encode(stdinVal) : stdinVal;
    this.stdinStrPos = 0;
  ...
  setStdinStr(str) {
    this.stdinStr = typeof str === 'string' ? new TextEncoder().encode(str) : str;
    this.stdinStrPos = 0;
  }

  addDirectory(path) {
    this.mem.check();
    const len = this.mem.write(this.exports.GetPathBuf(), path);
    this.exports.AddDirectoryNode(len);
  }
  ...
  addFile(path, contents) {
    ...
    const length = contents.byteLength !== undefined ? contents.byteLength : contents.length;
    this.mem.check();
    const pathLen = this.mem.write(this.exports.GetPathBuf(), path);
    const inode = this.exports.AddFileNode(pathLen, length);
  ...
  getFileContents(path) {
    this.mem.check();
    const pathLen = this.mem.write(this.exports.GetPathBuf(), path);
    const inode = this.exports.FindNode(pathLen);
    const addr = this.exports.GetFileNodeAddress(inode);
  ...
  host_read(fd, iovs, iovs_len, nread) {
    ...
    if (!self.sharedInt32) {
      // Fallback if no shared buffer (should not happen in cross-origin isolated environment)
      let size = 0;
      for (let i = 0; i < iovs_len; ++i) {
        const buf = this.hostMem_.read32(iovs);
        iovs += 4;
        const len = this.hostMem_.read32(iovs);
        iovs += 4;
        const bytesLeft = this.stdinStr.length - this.stdinStrPos;
        const lenToWrite = Math.min(len, bytesLeft);
        if (lenToWrite === 0) break;
        this.hostMem_.write(buf, this.stdinStr.subarray(this.stdinStrPos, this.stdinStrPos + lenToWrite));
        size += lenToWrite;
    ...
    for (let i = 0; i < iovs_len; ++i) {
      ...
      let writtenForThisIov = 0;
      while (writtenForThisIov < len) {
        let avail = self.sharedInt32[1] - self.sharedInt32[2];
        if (avail <= 0) {
          if (writtenForThisIov > 0) {
            break;
          }
          // No more input in shared buffer. Block and ask for more.
          self.sharedInt32[1] = 0;
          self.sharedInt32[2] = 0;
          Atomics.store(self.sharedInt32, 0, 1); // STATUS_WAITING
          if (self.port) {
            self.port.postMessage({id: 'waitingForStdin'});
          }
          
          // Block here
          Atomics.wait(self.sharedInt32, 0, 1);
          
          // Woken up! Let's check available bytes again.
          avail = self.sharedInt32[1] - self.sharedInt32[2];
          if (avail <= 0) {
            // EOF or empty line? If we got woken up but still 0, we treat as EOF.
            break;
          }
        }

        const chunk = Math.min(len - writtenForThisIov, avail);
        // Data starts at byte offset 128 in self.sharedUint8
        const dataStart = 128 + self.sharedInt32[2];
        const slice = self.sharedUint8.subarray(dataStart, dataStart + chunk);
        this.hostMem_.write(buf + writtenForThisIov, slice);

        self.sharedInt32[2] += chunk;
        writtenForThisIov += chunk;
        size += chunk;
      }
      
      if (writtenForThisIov < len) {
        break;
      }
    }
